...
const Header = () => {
    ...
    const Icon = open ? (
        <RiCloseLargeFill
            ...
            className={`text-3xl font-bold text-[var(--primary)] cursor-pointer transform transition-transform duration-300 ease-in-out ${open ? 'rotate-180' : 'rotate-0'}`}
        />
    ) : (
    ...
    return (
        <div className='w-full py-4 shadow-sm relative bg-white '>
            <div
               ref={refDiv}
                className={`fixed top-19 left-0 z-50 h-[90dvh] w-3/4 bg-white border border-[var(--gray)] rounded-tr-3xl rounded-b-2xl transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                            ...
                            {item.other ? (
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between w-full">
                                        ...
                                        <IoChevronDown className={`text-xl transition-transform ${openUtilities ? 'rotate-180' : ''}`} />
                                    </div>
                                </div>
                            ) : (
                                <Link
                                    to={item.url}
                                    className={`w-full flex items-center gap-4 p-2 rounded-md cursor-pointer ${pathName === item.url || pathName === item.main
